...
Classify through the versioned authority-delta resolver. `AWAIT_HUMAN` has
precedence only when it names non-empty concrete `missingAuthority`; incomplete
tests, stale evidence, repairable drift, or an implementation choice inside the
approved slice use `FAIL_CLOSED_REPAIR` or a bounded guard and never fabricate a
Human Gate. Record every deferred or blocked finding in the tracked
`TASK_LEDGER.md` Incidental Finding Register; the register does not authorize
follow-up work.

The required Completion Contract behavior and failing acceptance criteria may
not be deferred. They remain on the critical path unless a real authority delta
is recorded through the resolver.

Only the authority-gate recorder may write awaiting state. It atomically keeps
`current_stage` and `current_change.status` aligned, binds a deterministic gate
key and evidence receipt, and suppresses repeat questions for the same missing
authority. An unresolved real authority gate blocks completion and archive.
...
Automatic reclamation is registration-only. A trusted orchestrator supplies
the guarded apply safeguard only for a fresh exact `AUTO_CLEAN` plan after the
owner exits. `WAIT_OWNER` retries, `RETAIN` preserves evidence, and unsafe or
drifted state uses `FAIL_CLOSED_REPAIR` without awaiting markers. The lifecycle
engine's legacy `HUMAN_GATE` label is resolver input, not permission to persist
`AWAIT_HUMAN`; only concrete missing authority can produce that decision.
Cleanup uses no wildcard or recursive deletion and retains its receipt and
recoverable quarantine mapping. Physical purge remains a separate material
authority decision.
...
Stop for a Human Gate only when the resolver identifies a material authority
delta: an unresolved product/ownership decision, scope/write-set/public-contract
expansion, a new dependency or migration, an undeclared destructive/external
effect, severe or unknown material risk, or another concrete missing
permission. Technical failure stops mutation fail-closed as
`FAIL_CLOSED_REPAIR` without awaiting markers. Before asking, record the one
concrete missing authority and next question through the authority-gate
recorder; promote the answer into OpenSpec or the active ledger and resume.

For `model.*` work, a durable exact task/provider/model/existing-auth credential
policy/cost policy/serial grant is Standing Goal Execution Authority. A one-use
attempt receipt protects replay and evidence identity; it does not consume that
human authority. Record actual monetary cost and report it at completion, but
do not invent a per-call or per-retry currency gate. Failed or stale attempts
use bounded repair, evidence refresh, refreeze, and same-authority retry. Keep
the critical path focused: safely mitigate optional related improvements, mark
them `DEFER_AND_CONTINUE`, and summarize them at completion instead of opening
an intermediate task or Human Gate.

Checkpoint/compact is advisory and recoverable. Active-change verification is
not overall completion when approved work remains. A predeclared Standing
Milestone External Effects Contract may cover its exact reviewed
commit/push/tag/publication/readback/named-source-fast-forward/cache/project
refresh chain as one authority slice;
ordinary steps cannot infer that grant. PR, merge, force-push, archive,
undeclared install/update or migration, broad deletion, and unnamed targets
remain excluded unless separately and explicitly declared.

## Git Transport vs GitHub Control Plane

A gh authentication failure is not Git transport failure. For an authorized
push, use `git_transport_preflight.py` with contract-bound `git ls-remote`
readback, the expected remote commit, and fast-forward proof.
...
skill links drift. Applying cache refresh or project migration requires exact
authority; a current standing milestone may supply it only for its named target.
...
## Forbidden Without Current Authority

- destructive cleanup or broad architecture rewrite;
- public API or persistence-schema changes;
- production dependencies;
- bypassing failed tests;
- project migration apply, release sync apply, archive, commit, push, formal
  publication, cache/project refresh, or PR creation unless the exact effect and
  target are covered by a current approved slice or Standing Milestone External
  Effects Contract. A current milestone never grants archive, PR, merge, force,
  broad cleanup, or unnamed-consumer authority by implication.
