...
4. Prefer source-intake summaries and extracted Markdown under
   `_agent/source-intake/` when present.
5. Read only the raw source and task-relevant canonical notes.
...
2. Create or update a source summary with frontmatter. When `kb-import`
   already created one, preserve its provenance and review fields.
