...
def main() -> int:
    ...
    if status == "pending":
        return hook_response(
            repo,
            "codex-project-orchestrator: compact_status is pending. "
    ...
    if status == "skipped" and context.get("compact_skip_reason") in (None, "", "none"):
        return hook_response(
            repo,
            "codex-project-orchestrator: compact_status is skipped without a skip reason. "
            "Record one with record_compact_result.py before entering the next phase.",
        )
    if status in {"failed", "blocked"}:
        return hook_response(
            repo,
            f"codex-project-orchestrator: compact gate is {status}. "
