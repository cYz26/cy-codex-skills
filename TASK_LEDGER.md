...
- Evidence: the prior development-source Plugin Eval scored 68/D with one
  ...
  remains within the evaluator's good line-count range. The 2026-08-09
  standing-execution amendment recheck remains 77/C with trigger `407`, invoke
  `17,670`, deferred `57,124`, and active `18,077` tokens; its one failure and
  two warnings are static budget findings only, every skill remains within the
  evaluator's good line-count range, and all behavior/runtime verification is
  independently recorded. The Task 7.8 repair recheck on the exact release
  target is now 86/B with zero failures, three warnings, two informational
  findings, and active `17,529` tokens after a bounded reduction to the
  already-authorized `project-orchestrator` skill
- Affected Contract: the milestone validation contract requires zero failed
  Plugin Eval findings and records warning dispositions; the current 86/B
  result satisfies that gate without imposing a warning-free score threshold
...
- Disposition Reason: reducing the plugin-wide budget or Python complexity
  ...
  write set, so it is not one bounded guard for this behavior change. The human
  explicitly directed this run to keep monetary/model cost informational and
  to defer related non-blocking optimization rather than interrupt the active
  task; hard technical budgets in a task contract remain enforceable
...
### DF-IFL-011: Pre-promotion runner silently treats `--help` as a full run

- Finding ID: `DF-IFL-011`
- Disposition: `DEFER_AND_CONTINUE`
- Severity: low CLI usability and duplicate-work risk; no production, release,
  authority, or evidence-contract failure
- Evidence: invoking `dev/scripts/run_devflow_prepromotion_tests.py --help`
  started the full unittest suite because the script has no argument parser and
  ignores extra argv instead of printing usage or rejecting unknown options
- Affected Contract: none; the canonical no-argument pre-promotion command and
  its exact verification receipt remain unchanged
- Impact: an operator probing usage can unintentionally start an expensive
  duplicate local test run
- Current Mitigation: let the already-started read-only test run reach a safe
  boundary, then use the documented canonical no-argument command exactly;
  never infer help support for this runner
- Disposition Reason: adding argparse/help behavior is unrelated to standing
  model authority and does not block its resolver, guidance, refresh, or release
  validation
