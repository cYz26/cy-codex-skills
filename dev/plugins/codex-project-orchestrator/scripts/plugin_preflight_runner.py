...
def run_preflight(plugin_root: Path, marketplace: Path | None) -> dict[str, Any]:
    ...
    dependencies = check_dependencies(checks, plugin_root)
...
def check_manifest(checks: list[dict[str, Any]], manifest: dict[str, Any], hooks_path: Path) -> None:
    add_check(checks, "manifest.name", manifest.get("name") == "codex-project-orchestrator")
...
def check_dependencies(checks: list[dict[str, Any]], plugin_root: Path) -> dict[str, Any]:
    dependencies = dependency_report(plugin_root)
