...
the first CLI run for a repository when no trace file is supplied, and SHALL NOT
require or install claude-tap to complete the audit.

#### Scenario: First run without trace and claude-tap missing
- **WHEN** the user runs `context_fixer --repo <repo>` for a repository that has
  not previously seen dependency guidance
...
#### Scenario: First run without trace and claude-tap installed
- **WHEN** the user runs `context_fixer --repo <repo>` for a repository that has
  not previously seen dependency guidance
- **AND** no `--trace` file is supplied
...
- **WHEN** the user runs `context_fixer --repo <repo>` after dependency guidance
  has already been shown for that repository
