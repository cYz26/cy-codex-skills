...
The tool is named **Context Fixer**. It is a local-first, CLI-first context
auditor for Codex sessions, request traces, compaction timing, and AI project
configuration.

The Chinese working idea 「清道夫」 is retained only as naming background and brand
history. User-facing English naming should use Context Fixer.

Primary developer entry points:

- `PYTHONPATH=src python3.11 -m context_fixer --repo <repo>`
- `PYTHONPATH=src python3.11 -m context_fixer --repo <repo> --html report.html`
